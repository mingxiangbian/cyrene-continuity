...
10. Treat `cyrene_memory_propose` as review-first. It normally returns pending memory for explicit review; only strict low-risk project or procedural/system global memory may become active when v5 returns a named-policy, daily-capped, eval-gated `auto_promote` receipt.
...
20. Pending memory candidates are not active continuity memory. Do not use pending content as factual context until promoted with explicit approval and review-hash validation, or until a v5 named-policy `auto_promote` receipt confirms the candidate became active.
...
23. `Dream Deep` may recommend repeated independent evidence for review, but it must not activate high-risk, ambiguous, personal, relationship, affective, similar-project, or assistant-observed-only memory without explicit approval and review hash.
