...
export const BENCHMARK_CASES: readonly BenchmarkCase[] = [
  ...
  caseSpec({ id: 'T3-S-SCALE', tier: 'tier3', title: 'S scale fixture stays within latency and overhead thresholds', profiles: ['scale'], action: action('direct', 'runScaleFixture', 'S scale run'), expected: ['1 project with 50 active memories and 10 pending'], forbidden: ['fixture exceeds S scale budget'], metrics: ['continuityGetP50Ms', 'continuityGetP95Ms', 'memoryDbSizeBytes', 'memoryDbBytesPerMemory', 'scaleSRuntimeMs', 'activeMemoryGrowthPerRun', 'pendingGrowthPerRun', 'jsonlSizeBytes'] }),
  caseSpec({ id: 'T3-M-SCALE', tier: 'tier3', title: 'M scale fixture stays within latency and overhead thresholds', profiles: ['scale'], action: action('direct', 'runScaleFixture', 'M scale run'), expected: ['5 projects with 500 active memories and 100 pending'], forbidden: ['fixture exceeds M scale budget'], metrics: ['continuityGetP50Ms', 'continuityGetP95Ms', 'memoryDbSizeBytes', 'memoryDbBytesPerMemory', 'scaleMRuntimeMs', 'activeMemoryGrowthPerRun', 'pendingGrowthPerRun', 'jsonlSizeBytes'] }),
  caseSpec({ id: 'T3-L-SCALE', tier: 'tier3', title: 'L scale fixture stays within latency and overhead thresholds', profiles: ['scale'], action: action('direct', 'runScaleFixture', 'L scale run'), expected: ['20 projects with 5000 active memories and 1000 pending'], forbidden: ['fixture exceeds L scale budget'], metrics: ['continuityGetP95Ms', 'continuityGetP99Ms', 'indexStaleRate', 'memoryDbBytesPerMemory', 'scaleLRuntimeMs', 'activeMemoryGrowthPerRun', 'pendingGrowthPerRun', 'jsonlSizeBytes'] }),
  caseSpec({ id: 'T3-XL-SCALE', tier: 'tier3', title: 'XL scale fixture reports efficiency without entering release gate hot path', profiles: ['scale'], action: action('direct', 'runScaleFixture', 'XL scale run'), expected: ['100 projects with 50000 active memories and 5000 pending'], forbidden: ['XL run included in gate profile'], metrics: ['scaleXLRuntimeMs', 'memoryDbSizeBytes', 'memoryDbBytesPerMemory', 'benchmarkRuntimeMs', 'activeMemoryGrowthPerRun', 'pendingGrowthPerRun', 'jsonlSizeBytes'] }),
