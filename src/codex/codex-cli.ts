...
import { runJsonlRepairFromRoot } from '../memory/memory-repair.js'
import {
...
import {
  refreshCodexFastSummaryProjection,
...
export async function handleCodexCommand(input: { cwd: string; args: string[]; runtimeEntryPath?: string }): Promise<void> {
  ...
  if (command === 'memory' && input.args[1] === 'harvest-project') {
    ...
    const result = await runCodexProjectMemoryHarvest({
      ...
      dryRun: input.args.includes('--dry-run') ? true : undefined,
      apply: input.args.includes('--apply'),
  ...
  if (command === 'memory' && input.args[1] === 'jsonl' && input.args[2] === 'repair') {
    ...
    const memoryRoot = input.args.includes('--global')
      ? codexGlobalMemoryRoot()
      : codexProjectMemoryRoot((await identifyCodexProject(input.cwd)).projectId)
...
function parseOptionalCandidateHintReceipt(args: string[]): unknown {
  ...
  try {
    ...
    // Normalize JSON parse failures into the public CLI argument error below.
  }
