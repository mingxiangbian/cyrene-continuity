...
import { hasEnabledTomlTable } from './toml-lite.js'
...
export async function formatCodexMemoryDashboard(input: {
  ...
  return [
    ...
    '',
    'automation:',
    `  last automation run: ${dreamState.lastDreamAt ?? 'never'}`,
    `  next automation due: ${dreamState.nextDreamDueAt ?? 'unknown'}`,
    `  automation due: ${dreamState.dreamDue ? 'yes' : 'no'}`,
    dreamState.lastDreamStatus === undefined ? undefined : `  last automation status: ${dreamState.lastDreamStatus}`,
    dreamState.lastDreamError === undefined ? undefined : `  last automation error: ${dreamState.lastDreamError}`,
    '',
