...
import { readMemoryEdgesFromRoot } from '../memory/memory-store.js'
import type { MemoryEdge, SemanticMemory } from '../memory/types.js'
...
export function formatLifecycleProfileFromCoreMemory(input: {
  ...
  const core = input.memories
    .filter((memory) =>
      ...
      !supersededMemoryIds.has(memory.id) &&
...
function supersededProfileMemoryIds(memories: SemanticMemory[], relations: MemoryEdge[]): Set<string> {
  ...
  for (const relation of relations) {
    ...
    if (replacement === undefined || replaced === undefined) {
      continue
    }
...
function sanitizeProfileContent(content: string): string | null {
  const sanitized = redactReviewText(content).text
    .replace(/\s+/g, ' ')
    .trim()
