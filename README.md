...
- `cyrene_continuity_get`: read compact continuity context, response strategy,
  and principled dissent hints. It defaults to `mode=fast` and accepts
...
`cyrene_continuity_get` and `codex memory context-preview` use explicit context
modes:
...
- `balanced` uses richer active/session context without pending review
  visibility. It still hides pending candidates/counts/notices by default.
...
memory and profile projections. Similar-project session-hints are local
transferable guidance; they are not current-project facts, memory migration, or
promotion evidence.
...
These hints are disabled in `fast` mode and explicit/policy-gated in richer
modes. They are transferable session guidance, not facts about the current
project and not memory migration. `local_only`, personal, relationship, and
affective memories are excluded by policy and by the deterministic eval gate.
